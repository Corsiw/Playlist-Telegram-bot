...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if "bot_message" in context.user_data.keys():
        try:
            await context.bot.deleteMessage(
                context.user_data["bot_message"].chat.id,
    ...
    context.user_data["bot_message"] = await context.bot.send_photo(
...
async def build_start_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keys = [
        telegram.InlineKeyboardButton(pl[0], callback_data=pl[0])
        for pl in await db_get(sql)
    ]
    keyboard = [keys[i : i + 2] for i in range(0, len(keys), 2)]
    keyboard.append(
        [telegram.InlineKeyboardButton("Новый плейлист", callback_data="NEW_PLAYLIST")]
    )
    reply_markup = telegram.InlineKeyboardMarkup(keyboard)

    await context.bot.edit_message_caption(
        bot_message.chat.id, bot_message.message_id, caption=""
    )
    await context.bot.edit_message_reply_markup(
        bot_message.chat.id, bot_message.message_id, reply_markup=reply_markup
    )
...
async def start_menu_button(update: Update, context: CallbackContext):
    ...
    if update.callback_query.data == "0":
        ...
        if len(await db_get(sql)) >= MAX_PLAYLISTS:
            ...
            await context.bot.edit_message_caption(
                bot_message.chat.id, bot_message.message_id, caption="Назовите плейлист"
            )
            await context.bot.edit_message_reply_markup(
                bot_message.chat.id, bot_message.message_id, reply_markup=reply_markup
            )
        ...
        case "1":
            ...
            for track in await db_get(sql):
                await context.bot.send_audio(
                    chat_id=bot_message.chat.id, audio=track[0]
                )
            ...
            buttons = [
                (
                    telegram.InlineKeyboardButton(
                        f"{track[1]} - {track[0]}"[:30], callback_data=track[2]
                    )
                )
                for track in tracks
            ]
...
async def playlist_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query and update.callback_query.data == "0":
        ...
        for song in context.user_data["songs_to_add"]:
            ...
            sql.append(
                f"SELECT song_add({context.user_data['cur_playlist_id']}, '{song.file_id}', '{song.file_unique_id}', '{name}', '{author}', {song.duration})"
            )
...
async def playlist_remove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cache_key = (
        f"{bot_message.chat.id}:{context.user_data['cur_playlist_id']}:playlist_songs"
    )
    r_response = r.lrange(cache_key, 0, r.llen(cache_key))
    if r_response:
        tracks = [json.loads(r) for r in r_response]
...
async def playlist_rename(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.edit_message_caption(
        bot_message.chat.id, bot_message.message_id, caption=f"{update.message.text}"
    )
    await context.bot.edit_message_reply_markup(
        bot_message.chat.id, bot_message.message_id, reply_markup=reply_markup
    )
...
if __name__ == "__main__":
    ...
    main_handler = ConversationHandler(
        ...
        states={
            MAIN: [CallbackQueryHandler(start_menu_button)],
            PLAYLIST: [CallbackQueryHandler(playlist_button)],
            ADD: [
                CallbackQueryHandler(playlist_add),
                MessageHandler(filters.AUDIO, playlist_add),
            ],
            REMOVE: [CallbackQueryHandler(playlist_remove)],
            RENAME: [
