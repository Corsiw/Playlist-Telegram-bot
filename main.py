...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[telegram.InlineKeyboardButton("Начать!", callback_data="START")]]
    reply_markup = telegram.InlineKeyboardMarkup(keyboard)

    if "bot_message" in context.user_data:

        bot_message = context.user_data["bot_message"]

        await context.bot.edit_message_caption(bot_message.chat.id,
                                               bot_message.message_id,
                                               caption="Нажмите на кнопку, чтобы начать работу бота")
    ...
    context.user_data["bot_message"] = await context.bot.send_photo(update.message.chat.id, MAIN_PHOTO, "Нажмите на кнопку, чтобы начать работу бота", reply_markup=reply_markup,  read_timeout=999999)

    if str(context.user_data["bot_message"].chat.id) not in playlists.keys():
        playlists[str(context.user_data["bot_message"].chat.id)] = {}
...
async def build_playlist_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.edit_message_caption(bot_message.chat.id,
                                           bot_message.message_id,
                                           caption=context.user_data['cur_playlist_name'] + u"\u2800")
    await context.bot.edit_message_reply_markup(bot_message.chat.id,
                                                bot_message.message_id,
                                                reply_markup=reply_markup)
        ...
        case "1":
            for track in playlists[str(bot_message.chat.id)][context.user_data['cur_playlist_name']]:
                await context.bot.send_audio(chat_id=bot_message.chat.id,
                                             audio=playlists[str(bot_message.chat.id)][context.user_data['cur_playlist_name']][track])
...
async def playlist_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    playlists[str(bot_message.chat.id)][context.user_data['cur_playlist_name']][full_title] = update.message.audio.file_id
    with open("playlists.json", "w") as f:
        json.dump(playlists, f)
...
if __name__ == "__main__":
    ...
    main_handler = ConversationHandler(
        entry_points=[CallbackQueryHandler(build_start_menu),
                      CommandHandler("cansel", build_start_menu)],
        states={
            MAIN: [
